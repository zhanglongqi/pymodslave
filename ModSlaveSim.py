...
def ModServerFactory(args):
	...
	if args[0] == '-tcp':
		...
		try:
			...
			logger.error(f"Error while building TCP Server : {e}")
	elif args[0] == '-rtu':
		logger.info("Build RTU Server - Port: {0}, Baudrate: {1}, Bytesize: {2}, Parity: {3}, Stopbits : {4}".format(args[1], args[2], args[3], args[4], args[5]))
		try:
			_modServer = modbus_rtu.RtuServer(serial.Serial(port=int(args[1]), baudrate=int(args[2]), bytesize=int(args[3]), parity=args[4], stopbits=float(args[5]), xonxoff=0))
		except Exception as e:
			logger.error(f"Error while building RTU Server : {e}")
...
class ModSlaveSim(QtCore.QObject):
	...
	def __init__(self, modServer, slaveAddress, timeIntervalSim, no_coils=50, no_dis_inputs=50, no_input_regs=50, no_hold_regs=50):
