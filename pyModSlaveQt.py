...
from configparser import ConfigParser  # config file parser
...
#-------------------------------------------------------------------------------
class ModSlaveMainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
	...
	def _settings_RTU_show(self):
		...
		self._settingsRTU_dlg.exec()
		self._update_status_bar()

	def _settings_TCP_show(self):
		"""open TCP settings dialog"""
		self._settingsTCP_dlg.exec()
		self._update_status_bar()
	...
	def _update_status_bar(self):
		...
		if (self.ui.cmbModbusMode.currentText() == "TCP"):  #TCP server
			msg = "TCP : {0}:{1}".format(self._settingsTCP_dlg.tcp_ip, self._settingsTCP_dlg.tcp_port)
		elif (self.ui.cmbModbusMode.currentText() == "RTU"):  #RTU server
			msg = "RTU : {0}, {1}, {2}, {3}, {4}".format(self._settingsRTU_dlg.rtu_port, self._settingsRTU_dlg.baud_rate, self._settingsRTU_dlg.byte_size,
			...
			if (len(self._svr_args) > 0):  # check for valid no of parameters
				...
				if (self.svr is None):  #fail to build server
					...
					self.slv = simSlave.ModSlaveSim(self.svr, self.ui.sbSlaveID.value(),
													self.ui.spInterval.value() / 1000.0, self._settings_dlg.coils, self._settings_dlg.inputs, self._settings_dlg.input_regs,
					...
					if (self.slv is None):  #fail to build slave
						...
						self._mbdata_dlg.set_data_models(self.slv.coils_data_model, self.slv.dis_inputs_data_model, self.slv.input_regs_data_model, self.slv.hold_regs_data_model)
						self.slv.start()
		else:  #stop
	...
	def _load_params(self):
		...
		try:
			...
			self._logger.error("Parameters file is corrupted")
			return
	...
	def _save_params(self):
		...
		config.set('TCP', 'TCP_Port', f'{self._settingsTCP_dlg.tcp_port}')
		config.set('TCP', 'TCP_IP', self._settingsTCP_dlg.tcp_ip)
		#RTU Settings
		config.add_section('RTU')
		config.set('RTU', 'RTU_Port', f'{self._settingsRTU_dlg.rtu_port}')
		config.set('RTU', 'Baud', f'{self._settingsRTU_dlg.baud_rate}')
		config.set('RTU', 'DataBits', f'{self._settingsRTU_dlg.byte_size}')
		config.set('RTU', 'StopBits', f'{self._settingsRTU_dlg.stop_bits}')
		config.set('RTU', 'Parity', f'{self._settingsRTU_dlg.parity}')
		#Var Settings
		config.add_section('Var')
		config.set('Var', 'Coils', f'{self._settings_dlg.coils}')
		config.set('Var', 'DisInputs', f'{self._settings_dlg.inputs}')
		config.set('Var', 'InputRegs', f'{self._settings_dlg.input_regs}')
		config.set('Var', 'HoldRegs', f'{self._settings_dlg.hold_regs}')
		config.set('Var', 'TimeInterval', f'{self._time_interval}')
		config.set('Var', 'MaxNoOfBusMonitorLines', f'{self._settings_dlg.max_no_of_bus_monitor_lines}')
		#Save Settings
		config_file = open(self._params_file_name, 'w')
